import { collection, doc, getDocs, addDoc, deleteDoc, updateDoc, arrayUnion, arrayRemove, orderBy, query } from 'firebase/firestore'
...
export class TasksAPI {
  static async getTasks() {
    const response = await getDocs(query(collection(db, 'tasks'), orderBy('createdAt', 'desc')))

    return response.docs.map(doc => ({id: doc.id, ...doc.data()})) as TaskType[];
  }
  ...
  static async toggleComplete(id: TaskType['id'], value: TaskType['isComplete']) {
